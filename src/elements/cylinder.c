...
static double	cylinder_root(t_cylinder cylinder, t_ray ray)
{
	...
	c = vlen_squared(oc) - pow(dot(oc, cylinder.axis), 2) - cylinder.radius * cylinder.radius;
...
// 	top_center = vaddv(cylinder.center, vmultd(cylinder.axis, cylinder.height));
// 	distance_to_bottom = vlen(vsubv(point, bottom_center));
// 	distance_to_top = vlen(vsubv(point, top_center));
// 	if (distance_to_bottom < cylinder.radius)
// 		normal = unitv(vsubv(point, bottom_center));
// 	else if (distance_to_top < cylinder.radius)
// 		normal = unitv(vsubv(point, top_center));
// 	else
// 	{
...
// 		axis_to_point = vsubv(point, cylinder.center);
// 		distance_along_axis = dot(axis_to_point, cylinder.axis);
// 		projected_point = vaddv(cylinder.center, vmultd(cylinder.axis, distance_along_axis));
// 		normal = unitv(vsubv(point, projected_point));
// 	}
// 	return (normal);
// }
...
// 	hit.t = -1.0;
// 	top_center = vaddv(cylinder.center, vmultd(cylinder.axis, cylinder.height / 2));
// 	bot_center = vsubv(cylinder.center, vmultd(cylinder.axis, cylinder.height / 2));
// 	root = dot(vsubv(cylinder.center, ray.origin), cylinder.axis) / dot(ray.direction, cylinder.axis);
...
// 	if (root < 0 || root > cylinder.height)
// 		hit.t = -1.0;
// 	else if (dot(vsubv(hit.point, cylinder.center), cylinder.axis) >= 0 && dot(vsubv(hit.point, cylinder.center), cylinder.axis) <= cylinder.height)
// 	{
// 		if (vlen(vsubv(hit.point, cylinder.center)) <= cylinder.radius)
// 			hit = hit_plane((t_hittable){.plane = {cylinder.center, cylinder.axis}}, ray, interval);
// 		else if (vlen(vsubv(hit.point, vaddv(cylinder.center, vmultd(cylinder.axis, cylinder.height)))) <= cylinder.radius)
// 			hit = hit_plane((t_hittable){.plane = {vaddv(cylinder.center, vmultd(cylinder.axis, cylinder.height)), cylinder.axis}}, ray, interval);
// 	}
// 	return (hit);
// }